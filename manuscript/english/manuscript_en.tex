...
\author[2]{Juan Felipe Rodr\'iguez Galindo}
\author[3]{Octavio Jos\'e Salcedo Parra}
\affil[4]{Facultad de Ingenier\'ia, Maestr\'ia en Ciencias de la Informaci\'on y las Comunicaciones, Universidad Distrital Francisco Jos\'e de Caldas, Bogot\'a, Colombia}
...
The third group captures the \textit{leader-bus cascade dynamics}. Let $D^{L}$ denote the delay of the immediately preceding bus (leader) at the same stop. Inspired by the cascade mechanism described by \citet{1}, It is defined the relative delay differential to quantify whether the current bus is closing the gap with its leader:
\begin{equation}
  \delta_{rel} = D - D^{L}
\end{equation}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lccccr@{}}
    ...
    LR with L2      & 0.573 & 0.460 & \textbf{0.786} & 0.666 & 2.10 \\
    HGBT             & 0.649 & 0.607 & 0.699 & 0.776 & 3.20 \\
    RF ($T{=}100$)   & \textbf{0.656} & \textbf{0.996} & 0.486 & 0.758 & 9.85 \\
    XGBoost          & 0.651 & 0.600 & 0.713 & \textbf{0.781} & \textbf{1.50} \\
    \textbf{LightGBM}& 0.648 & 0.597 & 0.711 & \textbf{0.781} & 1.99 \\ \bottomrule
  \end{tabular}
  \label{tab:multimodel}
\end{table}

As shown in Table~\ref{tab:multimodel}, LightGBM and XGBoost achieve strong PR-AUC (0.781), outperforming the linear baseline (LR, PR-AUC=0.666). RF exhibits extreme precision (0.996) at the cost of lower recall. XGBoost is selected as the primary benchmarking model due to its low latency (1.50~ms).
...
conformal prediction with a nominal target coverage of 90\% ($\alpha=0.10$) on $N_{cal}=88{,}511$ calibration samples. Empirical coverage reached 99.69\%, exceeding the target guarantee and confirming conservative calibration. The quantile threshold was $q_{1-\alpha}=0.881$, and the average prediction set size was 1.20 labels, indicating that approximately 80\% of predictions carry a single high-confidence label while the remaining 20\% flag ambiguity by including both classes.
...
consumer hardware (representative of an Edge node in a DT). The model inference process---including feature scaling and evaluation of the LightGBM model---recorded an average time of \SI{1.99}{\ms} per record, with DuckDB feature extraction adding a negligible \SI{0.003}{\ms} per record (amortized), for a total end-to-end latency of \SI{1.99}{\ms} per record.
