...
\subsection{Evaluación de Cobertura Conformal}
Para validar la calibración de incertidumbre bajo variabilidad operativa de telemetría, evaluamos la predicción conformal tipo \textit{split} con cobertura nominal objetivo del 90\%. La cobertura empírica se mantuvo alineada con ese objetivo en regímenes con pérdida de telemetría: 91.2\% para $k=0$ (sin pérdida) y 89.4\% para $k=1$ (pérdida de un ciclo). En paralelo, el tamaño de los conjuntos conformales se adaptó dinámicamente a la incertidumbre, expandiéndose de un promedio de 1.05 etiquetas en condiciones nominales a 1.87 etiquetas durante caídas de telemetría. Este comportamiento es consistente con el diseño de despacho sensible al riesgo: se preserva la cobertura calibrada y se explicita la ambigüedad cuando la calidad de datos se degrada.
...
Este F1 de ablación (0.428) es intencionalmente menor que el 0.8227 de la Tabla \ref{tab:performance} porque ambas métricas responden preguntas distintas bajo regímenes de evaluación diferentes. La ablación se calculó sobre el corpus longitudinal global y altamente desbalanceado para aislar la sensibilidad a la profundidad secuencial, mientras que el F1 de la tabla de desempeño corresponde a una evaluación hold-out localizada por ruta, con balanceo de clases durante el ajuste del modelo y la calibración del umbral.
...
El salto aparente en densidad de clase es esperable: la proporción de 25.9\% reportada aquí ($513,213/1,979,230$) se calcula después del filtro de estado de transición ($B_{n-1}=0$), mientras que la cifra ``por debajo de 8\%'' de la Sección III-E refiere al flujo crudo pre-filtrado.
...
\subsubsection{Contextualización del Benchmark}
Para contextualizar esta latencia frente a motores analíticos comunes en operación de tránsito, la Tabla \ref{tab:benchmark_context_es} compara tiempos representativos de inferencia por registro. Respecto al baseline con Postgres, la ruta embebida con DuckDB mostró una aceleración observada de aproximadamente 45x en este estudio.

\begin{table}[htbp]
	\caption{Comparación de latencia de inferencia por registro entre backends}
	\centering
	\begin{tabular}{@{}lc@{}}
		\toprule
		\textbf{Backend} & \textbf{Latencia (ms)} \\ \midrule
		Pandas           & $\sim 10^{-1}$         \\
		Postgres         & $\sim 10^{-2}$         \\
		DuckDB           & 0.00042                \\ \bottomrule
	\end{tabular}
	\label{tab:benchmark_context_es}
\end{table}
